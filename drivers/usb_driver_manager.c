...
{
	if ( USB_DriverStruct == NULL )
	{
		USB_DriverStruct = driver;
...
ITCM_AREA_CODE uint32_t usb_device_driver_set_rx_buffer(uint8_t *rx_buf)
{
	USB_DriverStruct->rx_buf = rx_buf;
	return 0;
}

ITCM_AREA_CODE uint16_t usb_device_driver_get_rx_len(void)
{
...
ITCM_AREA_CODE uint32_t usb_device_driver_send(uint8_t* ptr, uint16_t len)
{
#ifdef	STM32U575xx
...
ITCM_AREA_CODE uint32_t usb_device_driver_pktreceived_callback(uint8_t* Buf, uint32_t Len)
{
	...
	memcpy(USB_DriverStruct->rx_buf,Buf,USB_DriverStruct->rx_buf_len);
