...
uint32_t	rx_len;
	...
	task_delay(PN5180_SEND_RECEIVE_DELAY*5);
...
uint32_t rx_len;
	...
	rx_len = pn5180_send_receive_spi(spi_nfc_Drv,4);
...
uint32_t rx_len = 0;
	...
	pn5180_write_register(spi_nfc_Drv,PN5180_CMD_WRITE_REGISTER,PN5180_REG_IRQ_CLEAR, 0xffff0f00);
	if ( spi_nfc_Drv->UID_len == NFC_ISO14443_UID7)
	{
		pn5180_write_register(spi_nfc_Drv,PN5180_CMD_WRITE_REGISTER_AND_MASK,PN5180_REG_CRC_RX_CONFIG, 0xfeffffff);
		spi_nfc_Drv->tx_data_ptr[0] = PN5180_CMD_SEND_DATA;
		spi_nfc_Drv->tx_data_ptr[1] = 0x00;
		spi_nfc_Drv->tx_data_ptr[2] = NFC_CMD_WRITE_NTAG213;
		spi_nfc_Drv->tx_data_ptr[3] = BlockNo;
		spi_nfc_Drv->tx_data_ptr[4] = Block[0];
		spi_nfc_Drv->tx_data_ptr[5] = Block[1];
		spi_nfc_Drv->tx_data_ptr[6] = Block[2];
		spi_nfc_Drv->tx_data_ptr[7] = Block[3];
		pn5180_send_spi(spi_nfc_Drv, 8);
		task_delay(20);
		...
		if ( rx_len )
		{
			...
			task_delay(5);
			pn5180_receive_spi(spi_nfc_Drv, rx_len);
			if ( spi_nfc_Drv->rx_data_ptr[0] != NFC_ISO14443_WRITE_SUCCESS)
				rx_len = 0;
			pn5180_write_register(spi_nfc_Drv,PN5180_CMD_WRITE_REGISTER_OR_MASK,PN5180_REG_CRC_RX_CONFIG,  0x01000000);
		}
	}
