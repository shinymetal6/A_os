...
__attribute__ ((aligned (32))) const int16_t	rom_dac_buffer[DAC_BUF_SIZE] =
{
	...
	0x16, 0x14, 0x13, 0x11, 0x0f, 0x0e, 0x0c, 0x0b,
	...
	0x0a, 0x0b, 0x0c, 0x0e, 0x0f, 0x11, 0x13, 0x14,
...
__attribute__ ((aligned (32))) int16_t	dac_buffer[DAC_BUF_SIZE];

__attribute__ ((aligned (32)))	DAC_DriverStruct_t DAC_Drv =
{
	.dac = &hdac1,
	.dac_timer = &htim7,
	.dac_sample_frequency = 48000,
...
void set_buffer(float vol)
{
uint32_t	i;
	for(i=0;i<DAC_BUF_SIZE;i++)
		dac_buffer[i] = (int16_t ) ((float )rom_dac_buffer[i] * vol );
}

void sample_process_1_init(uint32_t process_id)
{
	dac_register(&DAC_Drv);
}
...
void sample_process_1_basic_dac(uint32_t process_id)
{
uint32_t	wakeup,flags;
	...
	dac_init(&DAC_Drv);
	dac_start(&DAC_Drv);
	dac_timer_set(&DAC_Drv,FREQ*DAC_BUF_SIZE);
	while(1)
	{
...
#endif //#ifdef SAMPLEPROCESS_1_BASIC_DAC
