...
#include "../../kernel/system_default.h"
#include "../../kernel/HwDevices/hw_spi.h"
...
FRAME_BUFFER uint8_t  init_cmds1[] = {            // Init for 7735R, part 1 (red or green tab)
    15,                       // 15 commands in list:
    ST7735_SWRESET,   DELAY,  //  1: Software reset, 0 args, w/delay
      150,                    //     150 ms delay
    ST7735_SLPOUT ,   DELAY,  //  2: Out of sleep mode, 0 args, w/delay
...
uint8_t this_cmd[8] = {0,0,0,0,0x01,0,0,0};
    ...
    HAL_GPIO_WritePin(ST7735_RES_GPIO_Port, ST7735_RES_Pin, GPIO_PIN_SET);
    ST7735_Select();
    hw_spi_Send(LCD_HW_SPI_PORT,this_cmd, 8);
    ST7735_Unselect();
}
...
{
    HAL_GPIO_WritePin(ST7735_DC_GPIO_Port, ST7735_DC_Pin, GPIO_PIN_RESET);
    hw_spi_Send(LCD_HW_SPI_PORT,&cmd, sizeof(cmd));
}

static void ST7735_WriteData(uint8_t* buff, size_t buff_size)
{
    HAL_GPIO_WritePin(ST7735_DC_GPIO_Port, ST7735_DC_Pin, GPIO_PIN_SET);
    hw_spi_Send(LCD_HW_SPI_PORT,buff, buff_size);
}
...
void ST7735_ClearScreen(void)
{
    ...
    ST7735_SetAddressWindow(0, 0, ST7735_WIDTH-1, ST7735_HEIGHT-1);
    HAL_GPIO_WritePin(ST7735_DC_GPIO_Port, ST7735_DC_Pin, GPIO_PIN_SET);
    hw_spi_Send_DMA(LCD_HW_SPI_PORT, (uint8_t *)rect, ST7735_WIDTH*ST7735_HEIGHT*2);
    wait_dmadone(HW_SPI1);
    ST7735_Unselect();
}
